...
export const checkWCAGCompliance = (
  ...
  const aaThreshold = isLargeText
    ? WCAG_LEVELS.AA_LARGE
    : WCAG_LEVELS.AA_NORMAL;
  const aaaThreshold = isLargeText
    ? WCAG_LEVELS.AAA_LARGE
    : WCAG_LEVELS.AAA_NORMAL;
...
export const testThemeContrast = (): Array<{
  ...
  const combinations = [
    ...
    { fg: "muted-light", bg: "card", usage: "Muted text on card" },
    {
      fg: "overlord-red",
      bg: "deep-black",
      usage: "Brand color on dark background",
    },
    { fg: "overlord-red", bg: "surface-dark", usage: "Brand color on surface" },
    {
      fg: "light-text",
      bg: "overlord-red",
      usage: "White text on brand color",
    },
...
export const suggestImprovedColors = (
  ...
  if (fgRgb) {
    for (let i = 0; i <= 255; i += 15) {
      const lightFg = `#${Math.min(255, fgRgb.r + i)
        .toString(16)
        .padStart(2, "0")}${Math.min(255, fgRgb.g + i)
        .toString(16)
        .padStart(2, "0")}${Math.min(255, fgRgb.b + i)
        .toString(16)
        .padStart(2, "0")}`;
      const darkFg = `#${Math.max(0, fgRgb.r - i)
        .toString(16)
        .padStart(2, "0")}${Math.max(0, fgRgb.g - i)
        .toString(16)
        .padStart(2, "0")}${Math.max(0, fgRgb.b - i)
        .toString(16)
        .padStart(2, "0")}`;
  ...
  if (bgRgb) {
    for (let i = 0; i <= 255; i += 15) {
      const lightBg = `#${Math.min(255, bgRgb.r + i)
        .toString(16)
        .padStart(2, "0")}${Math.min(255, bgRgb.g + i)
        .toString(16)
        .padStart(2, "0")}${Math.min(255, bgRgb.b + i)
        .toString(16)
        .padStart(2, "0")}`;
      const darkBg = `#${Math.max(0, bgRgb.r - i)
        .toString(16)
        .padStart(2, "0")}${Math.max(0, bgRgb.g - i)
        .toString(16)
        .padStart(2, "0")}${Math.max(0, bgRgb.b - i)
        .toString(16)
        .padStart(2, "0")}`;
...
export const generateContrastReport = (): {
  ...
  const recommendations = results
    ...
    .map((r) => ({
      ...
      priority:
        r.compliance.ratio < 3 ? ("high" as const) : ("medium" as const),
    }));
